...
feature_test_noise () 
{
	...
	echo "$commands $1 --log=$logdir/$feature_cnt.log"
	$commands $1 --log=$logdir/$feature_cnt.log
	cat $logdir/$feature_cnt.log |grep -i "Noise detected at" > /dev/null
	evaluate_result $?
	echo "$commands $1" >> $logdir/$((feature_cnt-1)).log
}
...
#19 The latest file generated by alsabat.For the 20th test
latestfile=`ls -t1 /tmp/bat.wav.* | head -n 1` 
feature_test "--local -F 997 --file $latestfile -k $sigma_k" \
	     "local mode: analyze local file"
#20 roundtriplatency maybe need quiet environment without cooling fan.
feature_test "--roundtriplatency -E 1024 -B 2048" "round trip latency test"
echo "Need quiet environment without cooling fan"
sleep 10 #Avoid calling too fast, causing file generation errors
feature_test_noise "--snr-db 26 -k $sigma_k -r 192000" \
	     "noise detect threshold in SNR(dB)"
feature_test_noise "--snr-pc 15 -k $sigma_k -r 192000" \
