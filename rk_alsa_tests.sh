...
do       
	bash rk_alsa_test_tool.sh -t loopback -a 0 -d ${TEST_ACESSTYPE_TIME[$i]} #test_type=loopback access_type=RW_INTERLEAVED
        bash rk_alsa_test_tool.sh -t loopback -a 1 -d ${TEST_ACESSTYPE_TIME[$i]} #test_type=loopback access_type=MMAP_INTERLEAVED
	let "i += 1"
done

#ALSA_XS_FUNC_CAPTURE_ACCESSTYPE_RW_INTERLEAVED_01
bash rk_alsa_test_tool.sh -t capture -a 0 -F ALSA_M_FUNC_CAPTURE_ACCESSTYPE_RW_INTERLEAVED_01.snd;
bash rk_alsa_test_tool.sh -t playback -a 0 -F ALSA_M_FUNC_CAPTURE_ACCESSTYPE_RW_INTERLEAVED_01.snd
#ALSA_XS_FUNC_CAPTURE_ACCESSTYPE_MMAP_INTERLEAVED_01
bash rk_alsa_test_tool.sh -t capture -a 1 -F ALSA_M_FUNC_CAPTURE_ACCESSTYPE_MMAP_INTERLEAVED_01.snd;
bash rk_alsa_test_tool.sh -t playback -a 1 -F ALSA_M_FUNC_CAPTURE_ACCESSTYPE_MMAP_INTERLEAVED_01.snd
...
do       
	bash rk_alsa_test_tool.sh -t loopback -o 0 -d ${TEST_OPMODE_TIME[$i]} #test_type=loopback opmode=Blocking
        bash rk_alsa_test_tool.sh -t loopback -o 1 -d ${TEST_OPMODE_TIME[$i]} #test_type=loopback opmode=Non-Blocking
	let "i += 1"
done
...
do      
        bash rk_alsa_test_tool.sh -t capture -b ${TEST_BUFFERSIZE[$i]} -F ALSA_M_FUNC_CAP_BUFSZ_${TEST_BUFFERSIZE[$i]}.snd; #test_type=capture
        bash rk_alsa_test_tool.sh -t playback -b ${TEST_BUFFERSIZE[$i]} -F ALSA_M_FUNC_CAP_BUFSZ_${TEST_BUFFERSIZE[$i]}.snd #test_type=playback
        bash rk_alsa_test_tool.sh -t loopback -b ${TEST_BUFFERSIZE[$i]} #test_type=loopback
	let "i += 1"
done
...
do       
	bash rk_alsa_test_tool.sh -t capture -c 1 -d ${TEST_CHANNEL_TIME[$i]} -F ALSA_${TEST_CHANNEL_TIME[$i]}s_FUNC_CAPTURE_CHANNEL_1.snd
        bash rk_alsa_test_tool.sh -t playback -c 1 -d ${TEST_CHANNEL_TIME[$i]} -F ALSA_${TEST_CHANNEL_TIME[$i]}s_FUNC_CAPTURE_CHANNEL_1.snd
        bash rk_alsa_test_tool.sh -t loopback -c 1 -d ${TEST_CHANNEL_TIME[$i]}
        bash rk_alsa_test_tool.sh -t capture -c 2 -d ${TEST_CHANNEL_TIME[$i]} -F ALSA_${TEST_CHANNEL_TIME[$i]}s_FUNC_CAPTURE_CHANNEL_2.snd 
        bash rk_alsa_test_tool.sh -t playback -c 2 -d ${TEST_CHANNEL_TIME[$i]} -F ALSA_${TEST_CHANNEL_TIME[$i]}s_FUNC_CAPTURE_CHANNEL_2.snd
...
do      
        bash rk_alsa_test_tool.sh -t capture -p ${TEST_PERIODSIZE[$i]} -F ALSA_M_FUNC_CAP_PRDSIZE_${TEST_PERIODSIZE[$i]}.snd -d 60
        bash rk_alsa_test_tool.sh -t playback -p ${TEST_PERIODSIZE[$i]} -F ALSA_M_FUNC_CAP_PRDSIZE_${TEST_PERIODSIZE[$i]}.snd -d 60
...
do      
        bash rk_alsa_test_tool.sh -t capture -r ${TEST_SAMPLERATE[$i]} -F ALSA_M_FUNC_CAP_SMPRT_${TEST_SAMPLERATE[$i]}.snd -d 60
        bash rk_alsa_test_tool.sh -t playback -r ${TEST_SAMPLERATE[$i]} -F ALSA_M_FUNC_CAP_SMPRT_${TEST_SAMPLERATE[$i]}.snd -d 60
...
do	
	bash rk_alsa_test_tool.sh -t capture -f ${TEST_FORMAT[$i]} -F ALSA_M_FUNC_CAP_SMPFMT_${TEST_FORMAT[$i]}.snd
        bash rk_alsa_test_tool.sh -t playback -f ${TEST_FORMAT[$i]} -F ALSA_M_FUNC_CAP_SMPFMT_${TEST_FORMAT[$i]}.snd
...
echo "$startTime ---> $endTime" "Total running time:$sumTime_h hours,$sumTime_m minutes and $sumTime_s seconds"