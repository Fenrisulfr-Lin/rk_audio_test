...
echo "$startTime" > rk_alsa_format_test_result.log
...
feature_test () 
{
	echo "============================================"
	echo -e "\n$feature_cnt:|LOG| CMD=$*" \
		| tee -a rk_alsa_format_test_result.log
	echo "-------------------------------------------"
	log=`eval $*`
	evaluate_result $?
}
...
do      
	...
	then
		feature_test bash rk_alsa_test_tool.sh -t capture \
		-f ${TEST_FORMAT[$i]} -F ALSA_FORMAT_${TEST_FORMAT[$i]}.snd -d 5
		feature_test bash rk_alsa_test_tool.sh -t playback \
		-f ${TEST_FORMAT[$i]} -F ALSA_FORMAT_${TEST_FORMAT[$i]}.snd -d 5	
	else
